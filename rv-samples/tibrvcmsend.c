    ...
    tibrv_u32*  roundsNum)
{
    ...
    while ( i+2 <= argc && *argv[i] == '-' )    {
        if (strcmp(argv[i], "-service") == 0)        {
            ...
            *intervalNum = atof(argv[i+1]);            i+=2;
        }        else if (strcmp(argv[i], "-rounds") == 0)        {
            *roundsNum = atoi(argv[i+1]);            i+=2;
        }        else        {
            usage();
        }
    }
...
{
    tibrv_status err;
    printf("sendMsgCallback::Current Thread ID: %lu\n", GetCurrentThreadId());
    if (current_round == roundsNum)    {  //* If we've hit our limit, let's quit */
        tibrvEvent_Destroy(event);
    }
    err = tibrvcmTransport_Send(cmtransport, send_message);  //* Send the message */
    if (err == TIBRV_OK) {
...
int main(int argc, char **argv)
{
    ...
    if (err != TIBRV_OK)    {
        fprintf(stderr, "%s: Failed to add the input string to the message --%s\n", progname, tibrvStatus_GetText(err));
        exit(1);
    }
    ...
    if (err != TIBRV_OK)    {
        fprintf(stderr, "%s: Failed to set the send subject in the message --%s\n", progname, tibrvStatus_GetText(err));
        exit(1);
    }

    /* Set up a timer to send the message */
    current_round = 1;
    err = tibrvEvent_CreateTimer(&timerId, TIBRV_DEFAULT_QUEUE, sendMsgCallback, intervalNum, NULL);
    if (err != TIBRV_OK)    {
        fprintf(stderr, "%s: Failed to start the timer --%s\n", progname, tibrvStatus_GetText(err));
        exit(1);
    }
