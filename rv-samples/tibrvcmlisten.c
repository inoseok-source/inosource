...
void
advCB(tibrvEvent        event,      tibrvMsg          message,      void *            closure) {
    tibrv_status        err;
    const char          *string;
    const char          *name;
    ...
    if (err == TIBRV_OK)    {
        ...
        fprintf(stderr, "#### RVFT ADVISORY: %s \nAdvisory message is: %s\n", name, string);
    }
    return;
} /* advCB  */

void
my_callback( tibrvcmEvent event, tibrvMsg message, void* closure){
    tibrv_status        err;
    ...
    printf("my_callback::Current Thread ID: %lu\n", GetCurrentThreadId());
    err = tibrvMsg_GetSendSubject(message, &send_subject); // * Get the subject name to which this message was sent.
    if (err != TIBRV_OK)    {
        fprintf(stderr, "%s: Failed to get send subject --%s\n", progname, tibrvStatus_GetText(err));
    }    else    {
        err = tibrvMsg_GetReplySubject(message, &reply_subject); // * If there was a reply subject, get it.
        if (err != TIBRV_OK)        {
            if (err == TIBRV_NOT_FOUND)            {
                err = TIBRV_OK; // this is okay
            }            else            {
    ...
    if (err == TIBRV_OK)    {
        err = tibrvMsg_GetCMSender(message, &cmname); // * Get the correspondent name of the cm sender.
        if (err != TIBRV_OK)        {
            if (err == TIBRV_NOT_FOUND)            {
                err = TIBRV_OK;  // must be reliable protocol
            }            else            {
                fprintf(stderr, "%s: Error getting CM sender--%s\n",progname, tibrvStatus_GetText(err));
            }
        }        else        {
            certified = TIBRV_TRUE;  // Must be using certified delivery protocol
            err = tibrvMsg_GetCMSequence(message, &sequenceNumber);
            if (err == TIBRV_OK)            {
                listener_registered = TIBRV_TRUE; //* listener is registered for certified delivery */
            }            else            {
                if (err == TIBRV_NOT_FOUND)                {
                    err = TIBRV_OK;  //* listener isn't registered for certified delivery */
                }                else                {
    ...
    if (err == TIBRV_OK)    {
        err = tibrvMsg_ConvertToString(message, &theString); //* Convert the incoming message to a string.
    }
    if (err == TIBRV_OK)    {
        if (listener_registered)        {
            printf("subject=%s, reply=%s, message=%s, ", send_subject, ((reply_subject) ? reply_subject: "none"), theString);
            printf("certified sender=%s, sequence=%d\n", ((certified) ? "TRUE":"FALSE"), (tibrv_u32)sequenceNumber);
        }        else        {
            printf("subject=%s, reply=%s, message=%s, ", send_subject, ((reply_subject) ? reply_subject: "none"), theString);
            printf("certified sender=%s, receipt uncertified\n", ((certified) ? "TRUE":"FALSE"));
        }
    }
    err = tibrvcmEvent_ConfirmMsg(event,message);
    if (sequenceNumber == 3) {
        printf("*************sequenceNumber=%d, reply=%s, message=%s, ", (int)sequenceNumber, ((reply_subject) ? reply_subject: "none"), theString);
        err = tibrvcmEvent_ConfirmMsg(event,message);
    ...
    char**      cmnameStr)
{
    ...
    while ( i+2 <= argc && *argv[i] == '-' )    {
        if(strcmp(argv[i], "-service") == 0)        {
            ...
            *ledgerStr = argv[i+1];            i+=2;
        }        else if(strcmp(argv[i], "-cmname") == 0)        {
            *cmnameStr = argv[i+1];            i+=2;
        }        else        {
            usage();
        }
    }
...
int main(int argc, char** argv) {
    ...
    progname = argv[0];
    printf("main::Current Thread ID: %lu\n", GetCurrentThreadId());
    /*
     * Parse the arguments for possible optional parameter pairs.
     * These must precede the subject and message strings.
     */
    currentArg = get_InitParms( argc, argv, MIN_PARMS, &serviceStr, &networkStr, &daemonStr, &ledgerStr, &cmnameStr );
    err = tibrv_Open();  //* Create internal TIB/Rendezvous machinery */
    if (err != TIBRV_OK)    {
     ...
     */
    err = tibrvcmTransport_Create(&cmtransport, transport, cmnameStr, TIBRV_TRUE, ledgerStr, TIBRV_FALSE, NULL);
    if (err != TIBRV_OK)    {
     ...
     */
    err = tibrvEvent_CreateListener( &advId, TIBRV_DEFAULT_QUEUE, advCB, transport,"_RV.>",NULL);
    if(err != TIBRV_OK)    {
    ...
    while (currentArg < argc)    {
        ...
        err = tibrvcmEvent_CreateListener(&cmlistenId, TIBRV_DEFAULT_QUEUE, my_callback, cmtransport, argv[currentArg], NULL);
        if (err != TIBRV_OK)        {
            fprintf(stderr, "%s: Error %s listening to \"%s\"\n", progname, tibrvStatus_GetText(err), argv[currentArg]);
            exit(2);
        }
        err = tibrvcmEvent_SetExplicitConfirm(cmlistenId);
        currentArg++;
    }
    /*
